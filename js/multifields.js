...
var Multifields = (function(a) {
  ...
  __.prototype.on = function(e) {
    ...
    switch (e.type) {
      ...
      case 'click':
        switch (true) {
          case target.classList.contains('mf-add'):
            switch (true) {
              case parent.classList.contains('mf-thumb'):
                this.addThumb(parent);
                break;
              case parent.classList.contains('mf-actions'):
                this.addItem(parent.parentElement);
                break;
              case parent.classList.contains('mf-toolbar'):
                this.addGroup(parent.parentElement);
                break;
            }
            break;
          case target.classList.contains('mf-del'):
            switch (true) {
              case parent.classList.contains('mf-thumb'):
                this.delThumb(parent);
                break;
              case parent.classList.contains('mf-actions'):
                this.delItem(parent.parentElement);
                break;
              case parent.classList.contains('mf-toolbar'):
                this.delGroup(parent.parentElement);
                break;
            }
            break;
          case target.classList.contains('mf-thumb'):
            if (target.getAttribute('data-mf')) {
              this.openThumbWindow(e);
            } else {
              this.openBrowseServer(e);
            }
            break;
        }
        break;
  ...
  __.prototype.addThumb = function(el) {
    var _el = el.cloneNode(true);
    _el.id = 'thumb_' + new Date().getTime();
    _el.style = '';
    _el = this.replaceId(_el, true);
    el.insertAdjacentElement('afterend', _el);
    this.oncomplete();
  };

  __.prototype.delThumb = function(el) {
    el.parentElement.removeChild(el);
    this.oncomplete();
  };

  __.prototype.addItem = function(el) {
  ...
  __.prototype.delItem = function(el) {
    el.parentElement.removeChild(el);
    this.oncomplete();
  };

  __.prototype.addGroup = function(el) {
    var tpl = el.querySelector('.mf-toolbar select') || el.querySelector('.mf-toolbar input[type="hidden"]');
    if (tpl) {
      this.tpl = tpl.value;
      this.loadTemplate(function(data) {
        if (data) {
  ...
  __.prototype.delGroup = function(el) {
    if (el.classList.contains('multifields')) {
      if (el.children.length > 2 && confirm('Delete all ?')) {
        el.querySelectorAll('.mf-row, .mf-item').forEach(function(item) {
  ...
  __.prototype.replaceId = function(el, clear) {
    var i, t = new Date().getTime();
    clear = clear || false;
    el.querySelectorAll('[id]').forEach(function(item) {
      i = item.id.split('__');
      i[1] = t;
      item.id = i.join('__');
      i = item.name.split('__');
      i[1] = t;
      item.name = i.join('__');
      if (clear) {
        item.value = '';
  ...
  __.prototype.build = function(a) {
    ...
    [].forEach.call(a.children, function(el, i) {
      switch (true) {
        case el.classList.contains('mf-ghost'):
          ...
          if (el.classList.contains('mf-group')) {
            ...
            if (item) {
              ...
              switch (r['type']) {
                case 'checkbox':
                case 'option': {
                  var els = el.querySelectorAll('[id]:checked');
                  var values = [];
  ...
  __.prototype.openThumbWindow = function(e) {
    var _thumb, el = e.target, tvID = this.id, _this = this;
    if (parent.modx) {
      var el_clone = el.cloneNode(true);
      el_clone.querySelectorAll('[id]').forEach(function(item) {
        item.id = '_' + item.id;
      });
      _thumb = parent.modx.popup({
        height: 'auto',
        width: '80%',
        draggable: 0,
        overlay: 1,
        ...
        content: '' +
            ...
            ' </span>' +
            '</div>' +
            '<div id="open_' + el.id + '" class="multifields table">' + el_clone.innerHTML + '</div>'
      });
      _thumb.el.querySelector('#actions .btn-save').onclick = function() {
        _this.changeThumbWindow(el.id);
        _thumb.close();
      };
      _thumb.el.querySelector('#actions .btn-close').onclick = function() {
        _thumb.close();
      };
      _thumb.el.onchange = function(e) {
        if (e.target.getAttribute('tvname') === 'image' || e.target.getAttribute('tvname') === 'file') {
          _this.changeThumbWindow(el.id);
        }
      };
      _thumb.el.onkeyup = function(e) {
        if (e.target.getAttribute('tvname') === 'image' || e.target.getAttribute('tvname') === 'file') {
          _this.changeThumbWindow(el.id);
        }
      };
  ...
  __.prototype.changeThumbWindow = function(thumbID) {
    ...
    els.forEach(function(el) {
      var _el = thumb.getElementById(el.id.substr(1)), _type = _el.getAttribute('tvname');
  ...
  __.prototype.openBrowseServer = function(e) {
    if (e.target.tagName === 'I') {
      return;
    }
    e.preventDefault();
    var target, el = e.target, _el, type, _this = this;
    if (el.classList.contains('mf-ghost')) {
      _el = el.cloneNode(true);
      _el.classList.remove('mf-ghost');
      _el = this.replaceId(_el);
      el.insertAdjacentElement('beforebegin', _el);
    } else {
      _el = el;
    }
    type = _el.getAttribute('data-type');
    target = _el.querySelector('[tvname="' + type + '"]');
    this.BrowseServer(target.id, type + 's');
    target.onchange = function() {
      if (this.value !== '') {
        _el.style.backgroundImage = type === 'image' ? 'url("../' + this.value + '")' : 'url()';
      } else {
  ...
  __.prototype.BrowseServer = function(last, type) {
    lastFileCtrl = last;
    type = type || 'images';
    var o = 'toolbar=no,status=no,resizable=yes,dependent=yes';
    o += ',width=' + screen.width / 2;
    o += ',height=' + screen.height / 2;
    o += ',left=' + screen.width / 4;
    o += ',top=' + screen.height / 4;
    var oWindow = window.open(Multifields.urlBrowseServer + '?type=' + type, 'FCKBrowseWindow', o);
    oWindow.addEventListener('DOMContentLoaded', function(ev) {
      window.KCFinder = {};
  ...
  __.prototype.openRTEinWindow = function(id, tvID, options) {
      ...
      _richtext.frame.addEventListener('load', function() {
        ...
        form.onsubmit = function(e) {
          ...
          setTimeout(function() {
            ...
            document.getElementById(tvID).mf.oncomplete();
            _richtext.close();
          }, 100);
        };
...
!function(t) {
  ...
  var V, q, G, u, K, Z, h, Y, M, k, Q, o, J, $, l, s, c, f, P, tt, et, nt, ot, it, rt, I = [],
      at = !1,
      p = !1,
      lt = !1,
      d = [],
        ...
        if (t.scroll) {
          ...
          do {
            var u, f, p, g, v, m, b, w, _, y = h,
                ...
                x = D.width,
                N = D.height;
            if (_ = y !== ht ? (u = y.scrollWidth, f = y.scrollHeight, p = B(y), m = x < u && ('auto' === p.overflowX || 'scroll' === p.overflowX), b = N < f && ('auto' === p.overflowY || 'scroll' === p.overflowY), w = y.scrollLeft, y.scrollTop) : (u = ut.documentElement.scrollWidth, f = ut.documentElement.scrollHeight, p = B(ut.documentElement), m = x < u && ('auto' === p.overflowX || 'scroll' === p.overflowX || 'visible' === p.overflowX), b = N < f && ('auto' === p.overflowY || 'scroll' === p.overflowY || 'visible' === p.overflowY), w = ut.documentElement.scrollLeft, ut.documentElement.scrollTop), g = m && (vt(E - l) <= r && w + x < u) - (vt(T - l) <= r && !!w), v = b && (vt(S - s) <= r && _ + N < f) - (vt(C - s) <= r && !!_), !I[d]) {
              for (var X = 0; X <= d; X++) {
                I[X] || (I[X] = {});
              }
            }
  ...
  return ut.addEventListener('dragover', t), ut.addEventListener('mousemove', t), _t.prototype = {
    ...
    _computeIsAligned: function(t) {
      ...
      if (G && !b ? (E(), e = ut.elementFromPoint(t.clientX, t.clientY), x()) : e = t.target, e = yt(e, this.options.draggable, this.el, !1), !w && V && V.parentNode === this.el) {
        for (var n, o, i, r, a, l, s, c, d = this.el.children, h = 0; h < d.length; h++) {
          yt(d[h], this.options.draggable, this.el, !1) && d[h] !== e && (d[h].sortableMouseAligned = (n = t.clientX, o = t.clientY, i = d[h], r = this._getDirection(t, null), this.options, void 0, a = Mt(i), l = 'vertical' === r ? a.left : a.top, s = 'vertical' === r ? a.right : a.bottom, l < (c = 'vertical' === r ? n : o) && c < s));
        }
          ...
          if (s = yt(s, i.draggable, o, !1)) {
            if (h !== s) {
              if (e = xt(s, i.draggable), 'function' == typeof d) {
                if (d.call(this, t, s, this)) return Ct(n, c, 'filter', s, o, o, e), void (r && t.cancelable && t.preventDefault());
              } else if (d && (d = d.split(',').some(function(t) {
                if (t = yt(c, t.trim(), o, !1)) return Ct(n, t, 'filter', s, o, o, e), !0;
              }))) {
    ...
    _prepareDragStart: function(t, e, n, o) {
      ...
      n && !V && n.parentNode === a && (K = a, q = (V = n).parentNode, Z = V.nextSibling, h = n, J = l.group, Q = o, f = {
        ...
        r._disableDelayedDrag(), V.draggable = r.nativeDraggable, r._triggerDragStart(t, e), Ct(r, K, 'choose', V, K, K, Q), Dt(V, l.chosenClass, !0);
    ...
    _onDragStart: function(t, e) {
      ...
      (u = Nt(V)).draggable = !1, u.style['will-change'] = '', this._hideClone(), Dt(u, n.options.chosenClass, !1), n._cloneId = Xt(function() {
        n.options.removeCloneOnHide || K.insertBefore(u, V), Ct(n, K, 'clone', V);
      ...
      if (!gt && (!pt || t.rootEl || t.artificialBubble || bt(i, r))) {
        ...
        if (r !== V && (lt = !1), s && !a.disabled && (c ? d || (o = !K.contains(V)) : $ === this || (this.lastPutMode = J.checkPull(this, s, V, t)) && l.checkPut(this, s, V, t))) {
          ...
          if (0 === i.children.length || i.children[0] === G || (O = t, B = u, H = Mt(Et(i)), R = 'vertical' === B ? O.clientY : O.clientX, L = 'vertical' === B ? O.clientX : O.clientY, W = 'vertical' === B ? H.bottom : H.right, F = 'vertical' === B ? H.left : H.top, z = 'vertical' === B ? H.right : H.bottom, F < L && L < z && W < R && !V.animated)) {
            if (0 !== i.children.length && i.children[0] !== G && i === t.target && (r = Et(i)), r && (n = Mt(r)), c ? s._hideClone() : s._showClone(this), !1 !== St(K, i, V, e, r, n, t, !!r)) return i.appendChild(V), q = i, rt = null, U(), this._animate(e, V), r && this._animate(n, r), j();
            ...
            if (et !== r && (ot = null, f = Mt(r)['vertical' === u ? 'top' : 'left'], st = !1), E = r, x = u, N = (T = V) === V && rt || Mt(T), X = E === V && rt || Mt(E), Y = 'vertical' === x ? N.left : N.top, M = 'vertical' === x ? N.right : N.bottom, k = 'vertical' === x ? N.width : N.height, P = 'vertical' === x ? X.left : X.top, I = 'vertical' === x ? X.right : X.bottom, A = 'vertical' === x ? X.width : X.height, ot = (Y === P || M === I || Y + k / 2 === P + A / 2) && g || v || m || a.invertSwap || 'insert' === ot || 'swap' === ot ? ('swap' !== ot && (ct = a.invertSwap || v || at || m), p = function(t, e, n, o, i, r, a) {
              ...
              if (!r) {
                if (a && it < c * o) {
                  if (!st && (1 === nt ? d + c * i / 2 < s : s < h - c * i / 2) && (st = !0), st) {
                    f = !0;
                  } else {
              ...
              return 0;
            }(t, r, u, a.swapThreshold, null == a.invertedSwapThreshold ? a.swapThreshold : a.invertedSwapThreshold, ct, et === r), 'swap') : (y = r, C = xt(V, (D = a).draggable), S = xt(y, D.draggable), p = C < S ? 1 : -1, 'insert'), 0 === p) {
              return j();
            }
